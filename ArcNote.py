...
def show_about():
    ...
    try:
        icon_image = PhotoImage(file="icon.png")  # Ensure "icon.png" is in the working directory
...
def apply_hotkeys():
    root.unbind_all(config["save_hotkey"])
    root.unbind_all(config["exit_hotkey"])
    root.unbind_all(config["emoji_hotkey"])
...
def open_emoji_window():
    ...
    for i, emoji in enumerate(emojis):
        row = i // columns
        col = i % columns
        x1 = col * button_size + 5 # adds padding
        y1 = row * button_size + 5
        x2 = x1 + button_size - 10 # adds padding
        y2 = y1 + button_size - 10

        emoji_button = tk.Button(emoji_canvas, text=emoji, font=("Arial", 20), command=lambda e=emoji: insert_emoji(e)) # Increased font size
        emoji_button.place(x=x1, y=y1, width=x2-x1, height=y2-y1) #Uses place to allow for better control of the grid
